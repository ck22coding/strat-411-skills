...
7. **Sign the handoff.** Run `python3 ~/.claude/skills/411-case/scripts/deck_signature.py --sign {working-directory}/{case-prefix}-deck-spec.md`. This computes a sha256 hash over the argument-bearing fields (argument_flow, per-slide leadlines, per-slide callout items, recommendation statement + reasons + risks + next steps) and writes a `validation` block into the frontmatter. `/411-deck` Phase 1 verifies this signature before building — a missing or mismatched signature halts production and asks the user to confirm.
   - **Why signed:** The signature is the handoff contract. `/411-case` has already validated leadline phrasing (Phase 3 Step 5, Phase 4 Step 2), implication quality (Phase 4 Step 2), horizontal story (Phase 6 Step 6), and had the mock deck TA-reviewed (Phase 4.5). `/411-deck` trusts that validation work by verifying the signature — it does NOT re-check argument structure.
   - **If the user hand-edits the deck-spec** (e.g., fixes a typo in a leadline after signing), the signature will mismatch and `/411-deck` will halt. They can either re-run Phase 6 Step 4 to re-sign, or tell `/411-deck` to build anyway and accept the edits.
8. Tell the user: "Deck argument saved and signed at `{case-prefix}-deck-spec.md`. Review the file, then invoke `/411-deck` to produce the .pptx."
